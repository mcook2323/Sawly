...
export function AIDesignPrompt({ compact = false, initialValue = "", onPromptSubmit }: AIDesignPromptProps) {
  ...
  function submit(event: FormEvent) {
    ...
    startSubmission(() => {
      ...
      else router.push(`/designer?prompt=${encodeURIComponent(value)}`);
    });
  }
  ...
  return (
    <form onSubmit={submit} className={compact ? "" : "ai-prompt-shell"}>
      ...
      <div className={`mt-3 flex gap-3 ${compact ? "flex-col sm:flex-row" : "flex-col lg:flex-row"}`}>
        <textarea id={compact ? "design-prompt-compact" : "design-prompt"} rows={compact ? 2 : 3} value={prompt} onChange={(event) => setPrompt(event.target.value)} onKeyDown={keyDown} placeholder={compact ? "Describe the project, dimensions, material, or style…" : "Describe anything made of wood..."} className="ds-input min-h-24 flex-1 resize-none text-base leading-7 sm:text-lg" />
        <Button type="submit" variant="primary" disabled={!prompt.trim() || submitting} className="self-stretch px-7 lg:self-end">{submitting ? "Opening…" : compact ? "Continue" : "Generate Concepts"}</Button>
      </div>
      ...
      <p className="ds-caption mt-4">Sawly organizes your idea into safe concept directions. Verified build details remain deterministic.</p>
    </form>
  );
}
