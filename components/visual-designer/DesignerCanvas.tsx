...
interface DesignerCanvasProps { scene: VisualScene; selectedId: string | null; transformMode: TransformMode; zoom: number; snapEnabled: boolean; snapIncrement: number; onSelect: (id: string | null) => void; onTransform: (id: string, update: Partial<VisualSceneObject>) => void; }

const SceneObject = memo(function SceneObject({ object, mode, selected, transformMode, showDimensions, snapEnabled, snapIncrement, onSelect, onTransform }: { object: VisualSceneObject; mode: VisualMode; selected: boolean; transformMode: TransformMode; showDimensions: boolean; snapEnabled: boolean; snapIncrement: number; onSelect: (id: string) => void; onTransform: DesignerCanvasProps["onTransform"] }) {
  ...
  const visual = <mesh ref={mesh} name={object.name} position={[object.position.x, object.position.y, object.position.z]} rotation={[object.rotation.x, object.rotation.y, object.rotation.z]} scale={[object.scale.x, object.scale.y, object.scale.z]} visible={object.visible} onClick={(event: ThreeEvent<MouseEvent>) => { event.stopPropagation(); onSelect(object.id); }}><boxGeometry args={scale} /><meshStandardMaterial {...appearance} /><SelectionOutline selected={selected} /><DimensionOverlay object={object} selected={selected} visible={showDimensions} /></mesh>;
...
export function DesignerCanvas({ scene, selectedId, transformMode, zoom, snapEnabled, snapIncrement, onSelect, onTransform }: DesignerCanvasProps) {
  ...
  return <Canvas dpr={[1, 1.75]} gl={{ antialias: true, powerPreference: "high-performance" }} onPointerMissed={() => onSelect(null)} aria-label={`Interactive 3D model of ${scene.projectName}`}>
    ...
    <group>{scene.objects.map((object) => <SceneObject key={object.id} object={object} mode={scene.mode} selected={selectedId === object.id} transformMode={transformMode} showDimensions={scene.showDimensions} snapEnabled={snapEnabled} snapIncrement={snapIncrement} onSelect={onSelect} onTransform={onTransform} />)}</group>
