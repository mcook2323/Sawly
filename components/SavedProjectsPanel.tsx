...
export function SavedProjectsPanel({ projects, onDelete }: SavedProjectsPanelProps) {
  ...
  return (
    <section id="saved-projects" className="border-y border-[var(--color-border)] bg-[var(--color-surface)] px-5 py-12 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <div className="mb-5 flex items-end justify-between gap-4">
          <div>
            <p className="ds-eyebrow">Saved locally</p>
            <h2 className="ds-subheading mt-2">Saved projects</h2>
          </div>
          <p className="text-xs text-[#8b7f74]">Stored only in this browser</p>
        </div>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {projects.map((project) => (
            <article key={project.id} className="ds-card ds-card-interactive p-4">
              ...
              <div className="mt-4 flex gap-3 text-sm font-semibold">
                ...
                <button type="button" onClick={() => onDelete(project.id)} className="cursor-pointer text-[var(--color-danger)] hover:underline">Delete</button>
              </div>
            </article>
          ))}
        </div>
