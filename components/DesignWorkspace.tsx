...
export function DesignWorkspace({ initialPrompt, aiMode }: { initialPrompt: string; aiMode: SawlyAIMode }) {
  ...
  useEffect(() => {
    if (!aiEnabled || !hydrated || !activePrompt || editing || requestRoute === "custom-concept") return;
  ...
  }, [activePrompt, aiEnabled, answers, editing, hydrated, profile, requestRoute, retryCount]);
  ...
  return <main className="page-enter min-h-screen bg-[var(--color-canvas)] text-[var(--color-ink)]">
    ...
    <section className="ds-container ds-section">
      <div className="mx-auto max-w-6xl"><div className="flex flex-wrap items-center gap-3"><p className="ds-eyebrow">Guided Design Studio</p><Badge tone={aiEnabled ? "success" : "muted"}>{aiEnabled ? "AI-enhanced mode" : "Deterministic mode · no paid AI"}</Badge></div><h1 className="ds-heading mt-3">Let&apos;s shape the right project together.</h1><p className="ds-body mt-3 max-w-2xl">{aiEnabled ? "Sawly combines AI-enhanced project discovery with verified, deterministic project generators." : "Sawly uses local guided questions and verified template matching without calling paid AI services."} Recommendations never replace construction calculations.</p><div className="mt-8 max-w-5xl"><AIDesignPrompt key={activePrompt || "empty"} compact initialValue={activePrompt} onPromptSubmit={submitIdea} /></div></div>
...
function DesignRecommendations({ aiEnabled, resolution, saved, onSave, onGenerate, generating, generationError }: { aiEnabled: boolean; resolution: ReturnType<typeof scoreDesignProfile>; saved: boolean; onSave: () => void; onGenerate: () => void; generating: boolean; generationError: string | null }) {
