...
import { CUSTOM_CONCEPT_SCHEMA } from "@/lib/concepts/schema";
...
import { parseConceptProviderText } from "@/lib/concepts/schema";
...
export class OpenAICustomConceptProvider implements CustomConceptProvider {
  constructor(private client: OpenAI, private model: string, private timeoutMs = 45_000) {}
  async generate(request: CustomConceptRequest): Promise<CustomConceptPackage> {
    ...
    try {
      ...
      const concepts = parseConceptProviderText(response.output_text);
      return { schemaVersion: 1, id: randomUUID(), originalPrompt: request.prompt, createdAt: new Date().toISOString(), generationStatus: "text-ready", concepts: concepts.map((concept) => ({ ...concept, imageStatus: "queued", imageUrl: null, imageAttempts: 0, imageError: null, imageLastAttemptedAt: null, verificationStatus: "ai-concept-not-build-verified" })) };
    } finally { clearTimeout(timer); }
  }
}

export function createConceptProvider() { const key = process.env.OPENAI_API_KEY; return createPaidProvider(process.env.SAWLY_AI_MODE, isConceptProviderConfigured(key), () => new OpenAICustomConceptProvider(new OpenAI({ apiKey: key }), resolveOpenAITextModel(process.env.OPENAI_MODEL), Number(process.env.OPENAI_CONCEPT_TIMEOUT_MS || 45_000))); }
