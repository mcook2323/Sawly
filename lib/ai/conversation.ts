...
export function getConversationQuestions(profile: DesignProfile, answers: DesignAnswers): DesignQuestion[] {
  ...
  if (!profile.intendedUse && !answers.intendedUse) questions.push({ id: "intendedUse", prompt: "How do you plan to use it?", helpText: "A short phrase is enough—for example, family dining or extra patio seating.", type: "text", required: true });
  ...
  if (!profile.budget && !answers.budget) questions.push({ id: "budget", prompt: "What material budget are you aiming for?", type: "choice", required: false, options: [{ label: "Under $100", value: "under-100" }, { label: "$100–$250", value: "100-250" }, { label: "$250–$500", value: "250-500" }, { label: "Flexible", value: "flexible" }] });
  return questions;
}
