...
export function projectToScene(project: UniversalWoodProject): VisualScene {
  ...
  const objects = adapter === "verified-table" || (!verified && /table/.test(project.projectType) && !/workbench/.test(project.projectType)) ? tableObjects(project) : adapter === "verified-bench" || (!verified && /bench/.test(project.projectType)) ? benchObjects(project) : archetypeObjects(project);
